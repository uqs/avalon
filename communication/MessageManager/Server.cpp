...
const unsigned int Server::MAX_MSGSIZE_INCOMING = 2000;
const int  Server::READ_TIMEOUT = 60; // in seconds
const int  Server::BUFLENGTH = 256;
const char Server::SUCCESS[] = "ok";
const char Server::FAIL[] = "fail";

bool Server::done = false;
...
Server::Server(int port) : portno(port), info(cin.rdbuf()), error(cerr.rdbuf())
{
}
...
void Server::set_logfile(streambuf *logfile) {
	info.rdbuf(logfile);
	error.rdbuf(logfile);
}


void Server::log(std::string message, LogLevel severity)
{
	switch(severity) {
		case Normal:
			info << message << endl;
 ...
 */
void Server::handle_connection(int socket, struct in_addr *addr)
{
 ...
 * \exception std::string
 */
string Server::handle_data(const string data)
{
