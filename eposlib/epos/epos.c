...
/* test if motor is there */
int epos_test_bus(int id)
{
	int ret;
	can_message_t message;
	...
	message.content[6]= 0x00;
	message.content[7]= 0x00;
	ret = can_send_message(&message);
	// printf("in epos_test: return value is %d\n\n",ret);

	if(ret == -1)
	{
		// epos_read.node[id].is_there = 0;
		return 0;
	}
	else
	{
		// epos_read.node[id].is_there = 1;
		return 1;
	}
...
void epos_set_brake_state(int id, int state)
{
	can_message_t message;

	//  printf("Brake state changed!!\n");

	message.id = 0x600+id;
	message.content[0]= EPOS_WRITE_1_BYTE;
	message.content[1]= 0x79;
...
void epos_set_brake_mask(int id, int mask)
{
	can_message_t message;

	// printf("Brake mask set!\n");
	...
	message.content[4]= (mask & 0x000000ff);
	message.content[5]= ((mask & 0x0000ff00)>>8);
	message.content[6]= 0x00;
	message.content[7]= 0x00;
	can_send_message(&message);
}

void epos_set_brake_polarity(int id, int pol)
{
	can_message_t message;

	// printf("Brake polarity set!\n");
	...
	message.content[4]= (pol & 0x000000ff);
	message.content[5]= ((pol & 0x0000ff00)>>8);
	message.content[6]= 0x00;
	message.content[7]= 0x00;
	can_send_message(&message);
}
...
void epos_get_velocity_demand_value(int id)
{
	can_message_t message;
...
void epos_get_position_demand_value(int id)
{
	can_message_t message;
...
void epos_ask_poti_position(int id)
{
	// printf("Asking for the poti position...\n");
	can_message_t message;

	message.id = 0x600+id;
	message.content[0]= 0x40;
	message.content[1]= 0x04;
	message.content[2]= 0x60;
	message.content[3]= 0x00;
	message.content[4]= 0x00;
	message.content[5]= 0x00;
	...
	// printf("Ask poti-function: message.id = %x\n", message.id);
	can_send_message(&message);
}

void epos_get_poti_position(int id)
{
	// printf("Reading the Poti Value...\n");
	can_message_t message;

	message.id = 0x580+id;
	message.content[0]= 0x43;
	message.content[1]= 0x04;
	message.content[2]= 0x60;
	message.content[3]= 0x00;
	message.content[4]= 0x00;
	...
	// printf("Get poti-function: message.id = %x\n", message.id);
...
int epos_check_errors(int id)
{
	int index = 1;
	PDEBUG("ask for actual error history\n");
	can_message_t message;

	// printf("========= Asking for errors ===========\n\n");
	...
	while(index < 6)
	{
		...
		// printf("Error message is %x\n", epos_read.node[1].req_reset);
	}
...
{
	// printf("inside serial function!\n");

	PDEBUG("Get the EPOS serial number\n");
	can_message_t message;
...
int can_read_message_handler(const can_message_t* message)
{
	...
	// Deubg Stuff
#if 0
	printf("Hallo, wir sind im auswerter!\n");
	...
	else
	{
		...
		if ((message->id >= 0x581) && (message->id <= (0x581+EPOS_NUM_NODES-1)))
		{
			...
			if ((message->content[1]==0x62)
					&& (message->content[2]==0x60)
					&& (message->content[3]==0x00))
			{
				//PDEBUG("received position actual value ");
				epos_read.node[(message->id - 0x581)].demand_position //epos_read.node[(message->id - 0x581)]
					=message->content[4]
					+(message->content[5]<<8)
					+(message->content[6]<<16)
					+(message->content[7]<<24);
			...
			if ((message->content[1]==0x04)
					&& (message->content[2]==0x20)
					&& (message->content[3]==0x00))
			{
				// Serial Number has been asked for.

				// printf("Serial Number requested. Result is (not yet impl.)");
			}
			...
			if ((message->content[1]==0x6B)
					&& (message->content[2]==0x60)
					&& (message->content[3]==0x00))
			{
				epos_read.node[(message->id - 0x581)].demand_velocity
					=message->content[4]
					+(message->content[5]<<8);
			}
					...
					&& (message->content[2]==0x10))
			{
				...
				else
				{
					...
					if (errorcode != 0)
					{
						// Error is present -> requeset reset!
						epos_read.node[i].req_reset = 1;
						// printf("Flag was set!\n");
						errors_present = 1;
					}
		...
		{
			/*
			   printf("ID out of range!! look what we'll do...\n");
			   printf("message->id = %x\n", message->id);
			   i = 0;
			   while(i < 8)
			   {
			   printf("message->content[i] = %x\n", message->content[i]);
			   i++;
			   }
			   */

			if((message->content[1] == 0x40) &&
					(message->content[2] == 0x04) &&
					(message->content[3] == 0x60))
			{
				// printf("Poti value has been asked...\n");
			}

			if((message->content[1] == 0x43) &&
					(message->content[2] == 0x04) &&
					(message->content[3] == 0x60))
			{
				// printf("Poti value has been downloaded...\n");
			}
		}
	}
