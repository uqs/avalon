...
DDXVariable destinationData; //not used at the moment!!!
DDXVariable waypointData;
...
void * translation_thread(void * dummy)
{
	...
	while (1) {
		...
		if (1) //dataBoat.t_readto(boatData,0,0))
		{
            ...
            waypoints.Data[0].longitude = -73.7833; //jfk W° // double ( (rand()% 50000) / 1000.0); //generate fake gps data between 0 and 50 degrees
            waypoints.Data[0].latitude = 40.6333; //jfk N° //double ( (rand()% 50000) / 1000.0);

            //rtx_message("The latitude shoud be: %f \n",destinationCoord.latitude);
            
            boatData.position.longitude = -118.40; //LAX W° //double ( (rand()% 2000) / 1000.0) + destinationCoord.longitude;
            boatData.position.latitude = 33.95; //LAX N° //double ( (rand()% 2000) / 1000.0) + destinationCoord.latitude;
            ...
            //destinationData.t_writefrom(destinationCoord);
            ...
            //naviflags.set_new_waypoints = NAVIFLAGS_NEWCALC_YES;
            //naviflags.navi_status_aufruf = NAVIFLAGS_IDLE; //only to chekc if flag-checker is working properly!!
...
int main (int argc, char * argv[])
{
    ...
    DOC(DDX_STORE_REGISTER_TYPE (store.getId(), Destination));
	...
	DOB(store.registerVariable(destinationData, varname_dest, "Destination"));
	...
	rtx_message_routine ("Ctrl-C detected. Shutting down testaufruf für skipper...");
