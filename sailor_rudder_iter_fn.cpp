...
{    
    ...
    d_wind_r             = remainder((d_wind_r*180.0/M_PI),360.0)*M_PI/180.0;
    d2aoa                = remainder(((-d_wind_r+aoa)*180.0/M_PI),360.0)*M_PI/180.0;

    if (fabs(d2aoa) <= 2.0*M_PI/180.0)    
    {    c_sail_lift     = 0.0;}
    else if (fabs(d2aoa) <= 25.0*M_PI/180.0)
    {    c_sail_lift     = (2.24*fabs(d2aoa)-2.24*2.0*M_PI/180.0);}  
    else if ((fabs(d2aoa) > 25.0*M_PI/180.0) && (fabs(d2aoa) <= 90.0*M_PI/180.0))
    {    c_sail_lift     = -0.79*fabs(d2aoa)+1.1;}           
    else if ((fabs(d2aoa) > 90.0*M_PI/180.0) && (fabs(d2aoa) <= M_PI))
    {    c_sail_lift     = 0.0;}
    ...
    if (fabs(aoa) <= 2.0*M_PI/180.0)
    {     x_to_sail_coa = 0.0; y_to_sail_coa = 0; }
    else if (fabs(aoa) <= 40.0*M_PI/180.0)                           
    {     x_to_sail_coa = 0.2; y_to_sail_coa = 0.3; }
    else if (fabs(aoa) > 40.0*M_PI/180.0 && fabs(aoa) <= 70.0*M_PI/180.0)
    {     x_to_sail_coa = 0.3; y_to_sail_coa = 0.35; }
    else if (fabs(aoa) > 70.0*M_PI/180.0 && fabs(aoa) <= 120.0*M_PI/180.0)
    {     x_to_sail_coa = 0.4; y_to_sail_coa = 0.4;  }
    else if (fabs(aoa) > 120.0*M_PI/180.0 && fabs(aoa) <= 150.0*M_PI/180.0)
    {     x_to_sail_coa = 0.2; y_to_sail_coa = 0.3;  }
    else if (fabs(aoa) > 150.0*M_PI/180.0 && fabs(aoa) <= M_PI)
    ...
    d_water         = remainder((d_water*180.0/M_PI),360.0)*M_PI/180.0;
    k               = 0.5*dens_water*v_r_tot*v_r_tot*A_rudder;
    incid_angle     = -d_water + x;
    incid_angle     = remainder((incid_angle*180.0/M_PI),360.0)*M_PI/180.0;
    ...
    return k*(1.9*(1.0-exp(-fabs(remainder((-d_water + x*180.0/M_PI),360.0)*M_PI/180.0)*9.0))-2.4*fabs(remainder((-d_water + x*180.0/M_PI),360.0)*M_PI/180.0))*cos(-d_water+x)*cos(d_water)*signum(-vorzeichenR) + k*1.28*sin(fabs(remainder((-d_water + x*180.0/M_PI),360.0)*M_PI/180.0))*sin(-d_water+x)*signum(vorzeichenR)*sin(fabs(d_water))*signum(-vorzeichenRR) - Y_rudder_right;
    //return 0.5*dens_water*v_r_tot*v_r_tot*A_rudder;//k*(1.9*(1-exp(-fabs(remainder((-d_water + x*180.0/M_PI),360.0)*M_PI/180.0)*9))-2.4*fabs(remainder((-d_water + x*180.0/M_PI),360.0)*M_PI/180.0));
}
